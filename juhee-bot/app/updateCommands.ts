 ...
 * @description 디스코드 봇의 슬래시 커맨드를 등록/업데이트하는 스크립트
 * @author kevin1113dev
 * @version 1.0.0
 */
...
export default async function updateCommands() {
  ...
  try {
    ...
    if (DEV_MODE) {
      ...
      logger.debug("개발 서버용 슬래시 커맨드 등록 결과:", result);
      logger.info(`✅ 개발 모드: 서버 ID ${DEV_SERVER_ID}에 슬래시 커맨드가 ${(result as any[]).length}개 등록되었습니다.`);
    } else {
      const result = await rest.put(Routes.applicationCommands(CLIENT_ID), {
        body: Commands,
      });
      logger.debug("글로벌 슬래시 커맨드 등록 결과:", result);
      logger.info(`✅ 글로벌 모드: 모든 서버에 슬래시 커맨드가 ${(result as any[]).length}개 등록되었습니다.`);
    }
